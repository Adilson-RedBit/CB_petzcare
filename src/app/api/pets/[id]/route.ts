...
export async function PUT(
  ...
  try {
    ...
    const workerResponse = await proxyToWorker(`/api/pets/${id}`, {
      method: "PUT",
    ...
    if (workerResponse) {
      const errorData = await workerResponse.json().catch(() => ({ error: "Erro ao atualizar pet" }));
      return NextResponse.json(
        { error: errorData.error || "Erro ao atualizar pet" },
