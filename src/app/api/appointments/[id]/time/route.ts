...
export async function PATCH(
  ...
  try {
    ...
    const workerResponse = await proxyToWorker(`/api/appointments/${id}/time`, {
      method: "PATCH",
    ...
    if (workerResponse) {
      const errorData = await workerResponse.json().catch(() => ({ error: "Erro ao atualizar horário" }));
      return NextResponse.json(
        { error: errorData.error || "Erro ao atualizar horário" },
